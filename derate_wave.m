...
alpha0 = alpha0_dB*(log(10)/20);% 1 Np = 20 log10(e) dB ~= 8.685889638 dB % For intensity derating
alpha0_p = alpha0;% 1 Np = 20 log10(e) dB ~= 8.685889638 dB % For pressure derating
...
AmplitudeCorrection = exp(-(betaprime(f+1e-15)).*d);%function(f); %this is where 10^(-attenuation(f)/10) would go, except you'd use 10^(-attenuation(f)/20) for a pressure field and the other one for a power/intensity
